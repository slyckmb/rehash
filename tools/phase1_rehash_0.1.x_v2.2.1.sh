...
### 🔰 Guardrails Rehydrator v2.2.1
### 📦 Phase: dev/0.1.x only (no 0.2.x contamination)
### 🕐 Generated: $(date -Iseconds)
### 🌍 Assumes you are inside ~/dev/work/rehash/

REPO_NAME="rehash"
SOURCE_DIR="."        # 🔧 Use current dir as working root
REMOTE="origin"
GH_REPO="$REPO_NAME"

# ─────────────────────────────────────────────────────────────
# 🧠 Safety Check: Ensure no pre-existing .git from zip mess
if [ -d ".git" ]; then
  echo "✅ Existing .git found — proceeding inside $(pwd)"
else
